...
value (4, 'Sebastian', 'Battaglia', '25000999', md5('admin') ,1, 'Employee');
...
/* 1) Generar las estructuras necesarias para dar soporte a 4 sistemas diferentes :
    ...
    c) INFRAESTRUCTURA , que será el sistema que enviará la información de llamadas a
        la base de datos.
...
-- ======= CLIENTES =========
create user 'client'@'127.0.0.1' identified by 'cl1en7';
-- Permisos
grant select on utn_phones.users to 'client'@'127.0.0.1';
grant select on utn_phones.calls to 'client'@'127.0.0.1';
grant select on utn_phones.bills to 'client'@'127.0.0.1';
...
-- ==============================================================================================================
/* 2) La facturación se realizará por un proceso automático en la base de datos. Se debe
    programar este proceso para el primer día de cada mes y debe generar una factura por línea
    y debe tomar en cuenta todas las llamadas no facturadas para cada una de las líneas, sin
    tener en cuenta su fecha. La fecha de vencimiento de esta factura será estipulada a 15 días. */
-- ==============================================================================================================
SET GLOBAL event_scheduler = ON;
CREATE EVENT generate_bills
ON SCHEDULE EVERY 1 MONTH STARTS '2022-07-01 00:00:01'
...
-- ==============================================================================================================
/* 3) Generar las estructuras necesarias para el cálculo de precio de llamadas y las localidades (
        Ciudad de origen y ciudad de destino) al momento de inserción de llamadas, ya que solo se
        reciben los números de origen y destino , la fecha y la duración de la llamada  */
-- ==============================================================================================================
...
-- ==============================================================================================================
/*  4) Generar las estructuras necesarias para dar soporte a las consultas de llamadas por fecha y
    por usuario , debido a que tenemos restricción de que estas no pueden demorar más de dos
    segundos y tenemos previsto que tendremos 500.000.000 de llamadas en el sistema en el
    mediano plazo. Este reporte incluirá :
        i) Número de origen
        ii) Ciudad de origen
...
WHERE tlo.enabled = 1 and dni = 43143355 and call_date between '2022-07-10' and '2022-07-20';
